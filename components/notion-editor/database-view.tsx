...
import { DatabaseBlock, DatabaseViewType, DatabaseRecord, DatabaseProperty } from '@/lib/notion-types';
...
export const DatabaseView: React.FC<DatabaseViewProps> = ({ block, onUpdate }) => {
        ...
        <Table className="min-w-full">
          <TableHeader>
            <TableRow className="border-b border-[rgba(55,53,47,0.09)]">
              {block.properties.map((property) => (
                <TableHead key={property.id} className="bg-[#F1F1EF] text-[12px] uppercase tracking-wide font-medium text-foreground/80 sticky top-0">
                  {property.name}
                </TableHead>
              ))}
          ...
          <TableBody>
            {block.records.length === 0 ? (
              <TableRow>
                <TableCell colSpan={block.properties.length + 1} className="text-center py-8 text-muted-foreground">
                  No records yet
                </TableCell>
              </TableRow>
            ) : (
              block.records.map((record) => (
                <TableRow key={record.id} className="hover:bg-muted/20">
                  {block.properties.map((property) => (
                    <TableCell key={property.id} className="text-[14px] leading-[1.3] px-2 py-1 border-b border-[rgba(55,53,47,0.09)]">
                  ...
                  <Card key={record.id} className="p-3 border border-[rgba(55,53,47,0.09)] rounded-[6px]">
                    <div className="space-y-2">
                      <h4 className="font-medium text-[14px]">
                        {record.properties.title || 'Untitled'}
                      </h4>
        ...
        {block.records.map((record) => (
          <Card key={record.id} className="overflow-hidden rounded-[10px] border border-[rgba(55,53,47,0.09)]">
            ...
            <CardContent className="p-3">
              <h4 className="font-medium text-[14px] mb-2">
                {record.properties.title || 'Untitled'}
              </h4>
              <div className="flex flex-wrap gap-1">
                {Object.entries(record.properties)
  ...
  const renderView = () => {
    switch (block.viewType) {
      ...
      case 'list':
        return renderTableView(); // Similar to table but with different styling
      case 'calendar':
        return (
          <div className="space-y-4">
