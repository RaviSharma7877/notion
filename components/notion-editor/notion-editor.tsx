...
import { Loader2, Share2, XCircleIcon, Plus, GripVertical, Trash2, Copy, Move } from 'lucide-react';
import Image from 'next/image';
import EmojiPicker from '../global/emoji-picker';
import BannerUpload from '../banner-upload/banner-upload';
import CollaboratorSearch from '../global/collaborator-search';
import { Block, BlockType, NotionEditorState, BlockAction } from '@/lib/notion-types';
...
const NotionEditor = React.forwardRef<NotionEditorHandle, NotionEditorProps>(
  ({ dirDetails, dirType, fileId }, ref) => {
    ...
    // Create a new block
    const createBlock = useCallback((type: BlockType, content: string = ''): Block => {
      ...
      switch (type) {
        ...
        case 'toggle':
          return { ...baseBlock, type: 'toggle', isOpen: false };
        case 'quote':
        ...
        case 'video':
          return { ...baseBlock, type: 'video', content: '', url: '', provider: 'direct' };
        case 'audio':
          return { ...baseBlock, type: 'audio', content: '', url: '' };
        case 'file':
          return { ...baseBlock, type: 'file', content: '', url: '', filename: '', size: 0, mimeType: '' };
        case 'bookmark':
          return { ...baseBlock, type: 'bookmark', content: '', url: '' };
        case 'embed':
          return { ...baseBlock, type: 'embed', content: '', url: '', provider: 'other' };
        case 'synced_block':
    ...
    const addParagraphBelow = useCallback((afterBlockId: string, type?: BlockType, focusNewBlock: boolean = true) => {
      setEditorState((prev): NotionEditorState => {
        const newBlocks = [...prev.blocks];
        const afterIndex = newBlocks.findIndex(block => block.id === afterBlockId);
        const nextType: BlockType = (type === 'bulleted_list' || type === 'numbered_list' || type === 'todo' || type === 'toggle') ? type : 'paragraph';
    ...
    const details = useMemo(() => {
      ...
      const result = selectedDir ? selectedDir as WorkspaceDto | FolderDto | FileDto : dirDetails as WorkspaceDto | FolderDto | FileDto;
      
      // Use local banner URL if available, otherwise use the result
      const finalResult = localBannerUrl !== null 
        ? { ...result, bannerUrl: localBannerUrl }
        : result;
    ...
    const ensureTrailingBlankAndFocus = useCallback((focusNewBlank: boolean = true) => {
    ...
    useEffect(() => {
      console.log('NotionEditor details changed:', { 
        ...
        dirType, 
        fileId,
        fromState: state.workspaces.find(w => w.id === workspaceId)?.folders.find(f => f.id === folderId)?.files.find(f => f.id === fileId)
      });
                        ...
                        {(provided, snapshot) => (
                          ...
                          >
                            ...
                            <BlockToolbar
                              ...
                              onDuplicate={() => duplicateBlock(block.id)}
                              isVisible={false}
                              onOpenCommands={() => { setCommandTargetBlockId(block.id); setShowCommandPalette(true); }}
                            />
