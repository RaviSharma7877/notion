...
import { Block, BlockType, DatabaseBlock } from '@/lib/notion-types';
...
export const BlockRenderer: React.FC<BlockRendererProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      const isListBlock = block.type === 'bulleted_list' || block.type === 'numbered_list' || block.type === 'todo' || block.type === 'toggle';
    ...
    switch (block.type) {
      case 'paragraph':
        return (
          <div
            className={cn(
              "min-h-6 py-0 px-2 rounded hover:bg-muted/40 transition-colors",
          ...
          >
            {isEditing ? (
              ...
              <div className="text-[16px] leading-[24px]">
                {block.content || <span className="text-muted-foreground border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Type '/' for commands, or start writing...</span>}
              </div>
            )}
          </div>
        );
          ...
          >
            {isEditing ? (
              ...
              <div className={cn("", headingSize[block.type])}>
                {block.content || <span className="text-muted-foreground">Type '/' for commands, or start writing...</span>}
              </div>
            )}
          </div>
        );
      ...
      case 'toggle':
        return (
          <div
            className={cn(
              "min-h-6 py-1 px-2 rounded hover:bg-muted/30 transition-colors",
              isSelected && "bg-muted/40"
            )}
            onClick={() => onSelect()}
          >
            <div className="flex items-start gap-2">
              ...
              {isEditing ? (
                ...
                <div 
                  className="text-[16px] leading-[24px] flex-1 cursor-text"
                  onClick={() => setIsEditing(true)}
                >
                  {block.content || <span className="text-muted-foreground border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Type '/' for commands, or start writing...</span>}
                </div>
              ...
              <div className="ml-6 mt-2 space-y-[2px]">
                {block.children.map((child) => (
                  <BlockRenderer
                    ...
                    isSelected={false}
                    isFocused={false}
                    onUpdate={(updates) => onUpdate({ children: block.children?.map(c => c.id === child.id ? { ...c, ...updates } as Block : c) })}
          ...
          >
            {isEditing ? (
              ...
              <div className="text-[19px] leading-[28px]">
                {block.content || <span className="text-muted-foreground border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Quote</span>}
              </div>
            )}
          </div>
        );
          ...
          >
            ...
            {isEditing ? (
              ...
              <pre className="p-2 pt-7 text-[14px] leading-[20px] whitespace-pre-wrap font-mono">
                {block.content || <span className="text-muted-foreground border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Code</span>}
              </pre>
            )}
          </div>
        );
          ...
          >
            <div className="flex items-start gap-3">
              ...
              {isEditing ? (
                ...
                <div className="text-[16px] leading-[24px] flex-1">
                  {block.content || <span className="text-muted-foreground border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Callout</span>}
                </div>
              )}
            </div>
          </div>
      ...
      case 'image':
        return (
          <div
            className={cn(
              "py-2 px-2 rounded hover:bg-muted/20",
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Image</span>
                </div>
                {block.url ? (
                  <div className="space-y-2">
                    ...
                    />
                    {block.caption && (
                      <p className="text-sm text-muted-foreground border-none shadow-none focus-visible:ring-0 p-0 rounded-none">{block.caption}</p>
                    )}
                  </div>
                ) : (
                  <div className="space-y-2">
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium border-none shadow-none focus-visible:ring-0 p-0 rounded-none">Video</span>
                </div>
                {block.url ? (
                  <div className="space-y-2">
              ...
              <CardContent className="p-4">
                ...
                {block.url ? (
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <Input placeholder="Audio URL" value={block.url || ''} onChange={(e) => onUpdate({ url: e.target.value })} />
                    <Input placeholder="Caption" value={block.caption || ''} onChange={(e) => onUpdate({ caption: e.target.value })} />
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  <Input placeholder="File URL" value={block.url || ''} onChange={(e) => onUpdate({ url: e.target.value })} />
                  <Input placeholder="Filename" value={block.filename || ''} onChange={(e) => onUpdate({ filename: e.target.value })} />
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="mt-3 space-y-2">
                  <Input placeholder="Bookmark URL" value={block.url || ''} onChange={(e) => onUpdate({ url: e.target.value })} />
                  <Input placeholder="Title" value={block.title || ''} onChange={(e) => onUpdate({ title: e.target.value })} />
                  <Input placeholder="Description" value={block.description || ''} onChange={(e) => onUpdate({ description: e.target.value })} />
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      case 'equation':
        return (
          <div
            className={cn(
              "min-h-6 py-2 px-4 rounded hover:bg-muted/20",
              isSelected && "bg-muted/30"
            )}
            onClick={() => {
              onSelect();
      ...
      case 'table_of_contents':
        return (
          <div
            className={cn(
              "py-2 px-4 rounded hover:bg-muted/20",
              isSelected && "bg-muted/30"
            )}
            onClick={() => onSelect()}
          >
      ...
      case 'synced_block':
        return (
          <div
            className={cn(
              "py-2 px-2 rounded border hover:bg-muted/10",
              isSelected && "bg-muted/20"
            )}
            onClick={() => {
              onSelect();
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline">Add template</Button>
